...
Format.prototype.refresh = function()
{
    ...
    if (graph.isSelectionEmpty())
    {
        ...
        /*
         HACK TO DISPLAY THE NAME OF THE MODEL
         */
        if( ui.editor.filename != null ) {
            mxUtils.write(label, ui.editor.filename);
        } else {
    ...
    else
    {
        ...
        else
        {
            ...
            //
            // GULLY HACK: Edit the Style properties panel to provide data on the
            // various KEfED elements we edit.
            //
            mxUtils.write(label, mxResources.get('elementProperties'));
            //mxUtils.write(label, mxResources.get('style'));
            div.appendChild(label);

            var kefedPanel = div.cloneNode(false);
            kefedPanel.style.display = 'none';
            this.panels.push(new KefedPropertiesPanel(this, ui, kefedPanel));
            this.container.appendChild(kefedPanel);

            addClickHandler(label, kefedPanel, idx++);
        }
...
/**
 * GULLY HACK - ADDED NEW PANEL TO REPLACE STYLE PANEL
 */

/**
 * Adds the label menu items to the given menu and parent.
 */
KefedPropertiesPanel = function(format, editorUi, container)
{
    BaseFormatPanel.call(this, format, editorUi, container);
    this.init();
};

mxUtils.extend(KefedPropertiesPanel, BaseFormatPanel);

/**
 * Adds the label menu items to the given menu and parent.
 */
KefedPropertiesPanel.prototype.init = function()
{
    var ui = this.editorUi;
    var editor = ui.editor;
    var graph = editor.graph;
 ...
 * Adds the label menu items to the given menu and parent.
 */
KefedPropertiesPanel.prototype.addProperties = function(div)
{
    var ui = this.editorUi;
    var graph = ui.editor.graph;
    ...
    if( ss.vertices.length == 1 ) {
        ...
        var type = '';
        if( cell.style.search('kefed_entity') > -1)
            type = 'entity';
        else if( cell.style.search('kefed_fork') > -1)
            type = 'fork';
        else if( cell.style.search('kefed_measurement') > -1)
            type = 'measurement';
        else if( cell.style.search('kefed_parameter') > -1)
            type = 'parameter';
        else if( cell.style.search('kefed_process') > -1)
            type = 'process';
        else if( cell.style.search('kefed_constant') > -1)
            type = 'constant';
        else if( cell.style.search('kefed_branch') > -1)
            type = 'branch';

        /*
         initialize data for the element if none exists
         */
        var doc = mxUtils.createXmlDocument();
        var node = null;
        if (cell.value == "" && (type != 'branch' && type != 'fork')) {
            node = doc.createElement('kefed_element');
            node.setAttribute('uri', '');
            node.setAttribute('parent', '');
            node.setAttribute('label', '...');
            if (type == 'constant' || type == 'parameter' || type == 'measurement') {
                node.setAttribute('scale_uri', '');
                node.setAttribute('scale_label', '');
                node.setAttribute('scale_type', '');
            }
            cell.value = node;
        } else {
            node=cell.value
        ...
        /*
         Intializes the node type
         */
        var h2 = document.createElement('h2');
        mxUtils.write(h2, mxResources.get(type));
        div.appendChild(h2);
        ...
        /*
         Intializes the node label as uneditable text
         */
        var h3 = document.createElement('h3');
        mxUtils.write(h3, node.getAttribute('label', '...'));
        div.appendChild(h3);
        ...
        /*
         Intializes the uri data as a text box on the properties form
         */
        var uri_text = form.addText('URI', node.getAttribute('uri', ''));
        mxEvent.addListener(uri_text, 'change', function (e) {
            if (uri_text.value != '...')
                node.setAttribute('uri', uri_text.value);
        });

        var parent_text = form.addText('Parent Class', '');
        mxEvent.addListener(parent_text, 'change', function (e) {
            if (parent_text.value != '...')
                node.setAttribute('parent', parent_text.value);
        });

        if (type == 'constant' || type == 'parameter' || type == 'measurement') {

            var scale_type_select = form.addCombo('Scale Type');

            // Selects the toolbar icon if a selection change
            // is made in the corresponding combobox.
            mxEvent.addListener(scale_type_select, 'change', function() {
                if( scale_type_select.value == mxResources.get('scalarScale') ){
                    node.setAttribute('scale_type', 'scalarScale');
                } else if( scale_type_select.value == mxResources.get('nominalScale') ){
                    node.setAttribute('scale_type', 'nominalScale');
                } else if( scale_type_select.value == mxResources.get('categoricalScale') ){
                    node.setAttribute('scale_type', 'categoricalScale');
                } else if( scale_type_select.value == mxResources.get('ordinalScale') ){
                    node.setAttribute('scale_type', 'ordinalScale');
                } else if( scale_type_select.value == mxResources.get('structuredScale') ){
                    node.setAttribute('scale_type', 'structuredScale');
                } else if( scale_type_select.value == mxResources.get('ontologyTermScale') ){
                    node.setAttribute('scale_type', 'ontologyTermScale');
                } else if( scale_type_select.value == mxResources.get('naturalLanguageScale') ) {
                    node.setAttribute('scale_type', 'naturalLanguageScale');
                }
            });

            var scale_type = node.getAttribute('scale_type', '...')
            form.addOption(scale_type_select, mxResources.get('nominalScale'), (scale_type=='nominalScale'));
            form.addOption(scale_type_select, mxResources.get('scalarScale'), (scale_type=='scalarScale'));
            form.addOption(scale_type_select, mxResources.get('categoricalScale'), (scale_type=='categoricalScale'));
            form.addOption(scale_type_select, mxResources.get('ordinalScale'), (scale_type=='ordinalScale'));
            form.addOption(scale_type_select, mxResources.get('structuredScale'), (scale_type=='structuredScale'));
            form.addOption(scale_type_select, mxResources.get('ontologyTermScale'), (scale_type=='ontologyTermScale'));
            form.addOption(scale_type_select, mxResources.get('naturalLanguageScale'), (scale_type=='naturalLanguageScale'));

            var scale_name_text = form.addText('Scale Names', node.getAttribute('scale_name', ''));
            mxEvent.addListener(scale_name_text, 'change', function (e) {
                if (scale_name_text.value != '')
                    node.setAttribute('scale_name', scale_name_text.value);
            });

            var scale_unit_text = form.addText('Scale Unit Label', node.getAttribute('scale_units', ''));
            mxEvent.addListener(scale_unit_text, 'change', function (e) {
                if (scale_unit_text.value != '')
                    node.setAttribute('scale_units', scale_unit_text.value);
            });

            var scale_unit_uri_text = form.addText('Scale Unit URI', node.getAttribute('scale_unit_uri', ''));
            mxEvent.addListener(scale_unit_uri_text, 'change', function (e) {
                if (scale_unit_uri_text.value != '')
                    node.setAttribute('scale_unit_uri', scale_unit_uri_text.value);
            });
